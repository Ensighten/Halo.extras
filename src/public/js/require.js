...
(function () {
    ...
    // PATCH: Expose a getContext method
    req.getContext = function () {
      // TODO: Robustify outside of global require
    ...
    req.attach = function (url, context, moduleName, callback, type, fetchOnlyFunction) {
        ...
        if (isBrowser) {
          ...
          // Overwritten with XHR
          callback = callback || function () { context.completeLoad(moduleName) };

          function _xhrLoop() {
            // Don't loop if timed out
            if( isTimedOut ) {
              return;
            }

            var xhr = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            xhr.open('GET', url, !s.skipAsync[url]);
            xhr.onreadystatechange = function() {
              if (xhr.readyState === 4) {
                  ...
                  if (isFirefox) {
                    try {
                      ...
                      if (console && console.error) {
                        console.error('Error thrown by file ' + url + ' on line: ' + (e.lineNumber - err.lineNumber + 3), e);
                      }
                    }
                  } else {
                    window.eval(xhr.responseText + '\n//@ sourceURL=' + url + '\n');
                ...
                // TODO: Deal with text! plugin
                  // On error, try XHR again [timeout will handle our troubles]
                  setTimeout( _xhrLoop, 100 );
                }
              }
            };

            // Begin looping and set up timeout timer
            xhr.send(null);
            setTimeout(function () {
              isTimedOut = true;
            }, context.config.waitSeconds * 1000);
          }
          // Begin xhr loop and timeout
          _xhrLoop();
        } else if (isWebWorker) {
    ...
    // PATCH: Add a .has function for checking if a module exists or not
    require.has = function (name) {
      return !!req.getContext().loaded[name];
    };
